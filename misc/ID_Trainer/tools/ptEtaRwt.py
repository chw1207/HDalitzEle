...
# balanced reweighting (reweight to the class with highest statistics)
def df_balance_rwt(Mdf, SumWeightCol="instwei", NewWeightCol="NewWt", Classes=[""]):
    Mdf[NewWeightCol] = 1.
    sum_w, wei = [1.0] * len(Classes), [1.0] * len(Classes)
    for i, k in enumerate(Classes):
        sum_w[i] = sum(Mdf[SumWeightCol][Mdf.Class == k])
    maximum = max(sum_w)

    for i, k in enumerate(Classes):
        wei[i] = maximum / sum_w[i]
