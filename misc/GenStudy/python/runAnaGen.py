...
def get_parser():
    ...
    parser.add_argument(
        ...
        default = "/data4/chenghan/electron/skimTree/",
        type = str
    )
    return parser
...
def get_name(mode, mpoint):
    ...
    ntuple =  args.prefix + "job_{}_Dalitz_eeg_{}.root".format(ext, end)
    return ntuple
...
def main():
    ...
    for mode in decay_modes:
        for mpoint in mass_points:
            ...
            skimfile = "./miniTree/{}/miniTree_HDalitz_{}_eeg_{}_{}.root".format(args.era, mode, mpoint, args.era)

            # start executing the xAna
            try:
                xAnaGen(infile, skimfile, args.era, mode, 125)
...
if __name__ == "__main__" :
    ...
    ROOT.gROOT.ProcessLine(".L xAnaGen.C+O") # compile the macro with ACLiC
    from ROOT import xAnaGen
