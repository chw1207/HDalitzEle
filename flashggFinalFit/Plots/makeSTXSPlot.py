...
#colorMap = {'ggH':ROOT.kAzure-9,'qqH':ROOT.kOrange-4,'WH':ROOT.kGreen-6,'ZH':ROOT.kGreen-6,'ttH':ROOT.kMagenta-9,'tH':ROOT.kOrange}
...
if("minimal" in opt.mode)|("extended" in opt.mode):
...
for poi_idx in range(len(params[opt.mode])):
  ...
  if( poi == "r_tH")&( opt.doTHBox ):
    ...
    grs_ratio_TH.SetPointError(grs_ratio_TH.GetN()-1,0,0,0.25*abs(results[poi]['ErrorLo']),0.25*abs(results[poi]['ErrorHi']))
...
# Create canvas
canv = ROOT.TCanvas("canv","canv",1500,1100)
#pad1 = ROOT.TPad("pad1","pad1",0,0.35,1,1)
pad1 = ROOT.TPad("pad1","pad1",0,0.42777777,1,1)
pad1.SetTickx()
pad1.SetTicky()
pad1.SetLogy()
...
pad1.Draw()
pad2 = ROOT.TPad("pad2","pad2",0,0,1,0.515)
#pad2.SetTickx()
pad2.SetTicky()
pad2.SetTopMargin(0.000001)
pad2.SetBottomMargin(0.57777777777)
pad2.Draw()
padSizeRatio = 0.57222222222/0.515
...
if opt.mode == "stage1p2_maximal": haxes.GetYaxis().SetRangeUser(5e-2,nominal_max*5)
else: haxes.GetYaxis().SetRangeUser(1e-2,nominal_max*5)
...
# Add legend
leg = ROOT.TLegend(0.73,0.55,0.87,0.85)
leg.SetFillColor(0)
leg.SetFillStyle(0)
leg.SetLineColor(0)
leg.SetLineWidth(0)
...
gr_theory_dummy.SetLineWidth(1)
gr_theory_dummy.SetLineColor(14)
gr_theory_dummy.SetFillColor(17)
gr_theory_dummy.SetFillStyle(3144)
#leg.AddEntry(gr_theory_dummy,"Uncertainty in SM pred.","FL")
leg.AddEntry(gr_theory_dummy,"SM prediction","FL")
leg.Draw("SAME")
# Add box for values
...
if "extended" in opt.mode: 
  lat2.DrawLatex(0.13,0.32,"STXS stage 1.2: minimal")
  if "obs" in opt.inputJson: lat2.DrawLatex(0.13,0.25,"m_{H} = 125.38 GeV,  #it{p}_{SM} = 70%")
  else: lat2.DrawLatex(0.13,0.25,"m_{H} = 125.38 GeV")
elif "minimal" in opt.mode: 
  lat2.DrawLatex(0.13,0.32,"STXS stage 1.2: minimal (old)")
  if "obs" in opt.inputJson: lat2.DrawLatex(0.13,0.25,"m_{H} = 125.38 GeV,  #it{p}_{SM} = TBC%")
  else: lat2.DrawLatex(0.13,0.25,"m_{H} = 125.38 GeV")
elif "maximal" in opt.mode: 
  lat2.DrawLatex(0.13,0.32,"STXS stage 1.2: maximal")
  if "obs" in opt.inputJson: lat2.DrawLatex(0.13,0.25,"m_{H} = 125.38 GeV,  #it{p}_{SM} = 31%")
  else: lat2.DrawLatex(0.13,0.25,"m_{H} = 125.38 GeV")
elif "stage0" in opt.mode:
  lat2.DrawLatex(0.13,0.32,"STXS stage 0")
  if "obs" in opt.inputJson: lat2.DrawLatex(0.13,0.25,"m_{H} = 125.38 GeV,  #it{p}_{SM} = 35%")
  else: lat2.DrawLatex(0.13,0.25,"m_{H} = 125.38 GeV")
...
if opt.mode == "stage1p2_extended": haxes_r.GetXaxis().SetLabelSize(0.06*padSizeRatio)
elif opt.mode == "stage1p2_minimal": haxes_r.GetXaxis().SetLabelSize(0.045*padSizeRatio)
...
haxes_r.GetXaxis().SetLabelOffset(0.015)
...
if opt.mode == "stage1p2_maximal": haxes_r.GetYaxis().SetRangeUser(-0.49,2.9)
elif opt.mode == "stage1p2_extended": haxes_r.GetYaxis().SetRangeUser(-0.49,2.9)
...
lat.DrawLatex(0.1,0.93,"#bf{CMS}")
lat3 = ROOT.TLatex()
lat3.SetTextFont(42)
lat3.SetTextAlign(31)
lat3.SetNDC()
lat3.SetTextSize(0.045)
lat3.DrawLatex(0.9,0.93,"137 fb^{-1} (13 TeV)")

# Save canvas
if "obs" in opt.inputJson:
  canv.SaveAs("/eos/home-j/jlangfor/www/CMS/hgg/stxs_runII/Dec20/final_new/checks/FR_v2/stxs_summary_%s_obs_v2.pdf"%opt.mode)
  canv.SaveAs("/eos/home-j/jlangfor/www/CMS/hgg/stxs_runII/Dec20/final_new/checks/FR_v2/stxs_summary_%s_obs_v2.png"%opt.mode)
else:
