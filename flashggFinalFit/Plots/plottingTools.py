...
def makeSplusBPlot(workspace,hD,hSB,hB,hS,hDr,hBr,hSr,cat,options,dB=None,reduceRange=None):
  ...
  if reduceRange is not None:
    ...
    for h_ipdf in [hSB,hB]:
    # for h_ipdf in [hSB,hB,hS]:
      for h in h_ipdf.itervalues(): h.GetXaxis().SetRangeUser(reduceRange[0],reduceRange[1])
  
  canv = ROOT.TCanvas("canv_%s"%cat,"canv_%s"%cat,800,800)
  pad1 = ROOT.TPad("pad1_%s"%cat,"pad1_%s"%cat,0, 0.3, 1, 1.0)
  pad1.SetTickx()
  pad1.SetTicky()
  ...
  pad1.Draw()
  pad2 = ROOT.TPad("pad2_%s"%cat,"pad2_%s"%cat,0, 0, 1, 0.3)
  pad2.SetTickx()
  pad2.SetTicky()
  ...
  h_axes = hD.Clone()
  h_axes.Reset()
  if options.doBands: h_axes.SetMaximum((hD.GetMaximum()+hD.GetBinError(hD.GetMaximumBin()))*1.5)
  ...
  else: leg = ROOT.TLegend(0.58, 0.56, 0.86, 0.76)
  leg.SetFillStyle(0)
  leg.SetLineColor(0)
  ...
  if options.unblind:
    ...
    leg.AddEntry(hS['pdfNBins'],"Signal Model #times {}".format(sigScale),"fl")
  if options.doBands:
  ...
  if options.unblind:
    ...
    hS['pdfNBins'].Scale(float(sigScale))
  ...
  # else: lat0.DrawLatex(0.17, 0.76,"#scale[0.85]{#mu = 1.0}")
  CMS_lumi(pad1, 5, 0, "137.1 fb^{-1}", 2017, True, "Work-in-progress", "", "")
  canv.Update()
  pad1.RedrawAxis()
  ...
  if options.unblind:
    ...
    hSr.Scale(float(sigScale)*padSizeRatio)
