...
WeightHandler::~WeightHandler(){
    ...
    etabins.clear();
    etabins.shrink_to_fit();

    r9bins.clear();
    r9bins.shrink_to_fit();
    
    values_2D.clear();
    values_2D.shrink_to_fit();
    
    uncert_2D.clear();
    uncert_2D.shrink_to_fit();

    values_3D.clear();
    values_3D.shrink_to_fit();
    
    uncert_3D.clear();
    uncert_3D.shrink_to_fit();
}


void WeightHandler::Init(std::string name_, YAML::Node sfcfg_){
    name = name_;
    sfcfg = sfcfg_;
    bool readfile = (sfcfg["file"]) ? true : false;
    bool is3Dvalue = (sfcfg["pt"])   ? true : false;
    if (readfile){
        auto fName = sfcfg["file"].as<std::string>();
        InFile = TFile::Open(fName.c_str() , "READ");
        if (!InFile || InFile->IsZombie())
            throw std::runtime_error(Form("WeightHandler::Init (%s): TFile::Open failed: %s", name.c_str(), fName.c_str()));
        
        auto hName = sfcfg["hist"].as<std::string>();
        SFs = (TH2F*) InFile->Get(hName.c_str());
        if (!SFs)
            throw std::runtime_error(Form("WeightHandler::Init (%s): TFile::Get() failed: %s", name.c_str(), hName.c_str()));

        SFs->SetDirectory(nullptr);
        InFile->Close();
    }
    else if (!is3Dvalue){
        etabins = sfcfg["absEta"].as<std::vector<float>>();
        r9bins  = sfcfg["fullR9"].as<std::vector<std::vector<float>>>();
        values_2D  = sfcfg["values"].as<std::vector<std::vector<float>>>();
        uncert_2D  = sfcfg["uncert"].as<std::vector<std::vector<float>>>();
    }
    else{
        etabins = sfcfg["absEta"].as<std::vector<float>>();
        r9bins  = sfcfg["fullR9"].as<std::vector<std::vector<float>>>();
        ptbins  = sfcfg["pt"].as<std::vector<float>>();
        values_3D  = sfcfg["values"].as<std::vector<std::vector<std::vector<float>>>>();
        uncert_3D  = sfcfg["uncert"].as<std::vector<std::vector<std::vector<float>>>>();
    }
}


float WeightHandler::GetSFFromHgg(float sceta, float full5x5r9, float pt){
    int etabin = utils::FindBins(etabins, fabs(sceta));
    if (etabin == -1)
        throw std::runtime_error(Form("WeightHandler::GetSFFromHgg (%s): SCEta = %f, out of absEta bins range", name.c_str(), fabs(sceta)));

    int r9bin  = utils::FindBins(r9bins.at(etabin), full5x5r9);
    if (r9bin == -1)
        throw std::runtime_error(Form("WeightHandler::GetSFFromHgg (%s): full5x5r9 = %f, out of fullR9 bins range", name.c_str(), full5x5r9));

    int ptbin;
    float sf = 1;
    if (values_3D.size() > 0){
        ptbin = utils::FindBins(ptbins, pt);
        if (ptbin == -1)
            throw std::runtime_error(Form("WeightHandler::GetSFFromHgg (%s): pt = %f, out of pt bins range", name.c_str(), pt));
        sf = values_3D.at(etabin).at(r9bin).at(ptbin);
    }
    else
        sf = values_2D.at(etabin).at(r9bin);
    return sf;
}


float WeightHandler::GetSFErrFromHgg(float sceta, float full5x5r9, float pt){
    int etabin = utils::FindBins(etabins, fabs(sceta));
    if (etabin == -1)
        throw std::runtime_error(Form("WeightHandler::GetSFFromHgg (%s): abs(SCEta) = %f, out of range of absEta bins", name.c_str(), fabs(sceta)));

    int r9bin  = utils::FindBins(r9bins.at(etabin), full5x5r9);
    if (r9bin == -1)
        throw std::runtime_error(Form("WeightHandler::GetSFFromHgg (%s): full5x5r9 = %f, out of range of fullR9 bins", name.c_str(), full5x5r9));
    ...
    if (uncert_3D.size() > 0){
        ...
        if (ptbin == -1)
            throw std::runtime_error(Form("WeightHandler::GetSFFromHgg (%s): pt = %f, out of pt bins range", name.c_str(), pt));
        sferr = uncert_3D.at(etabin).at(r9bin).at(ptbin);
    }
    else
        sferr = uncert_2D.at(etabin).at(r9bin);
    return sferr;
}


float WeightHandler::GetSFFromEGM(float x, float y){
    if (!SFs)
        throw std::runtime_error(Form("WeightHandler::GetSFFromEGM (%s): null histogram", name.c_str()));

    int xbin = TMath::Max(1, TMath::Min(SFs->GetNbinsX(), SFs->GetXaxis()->FindBin(x)));
    int ybin = TMath::Max(1, TMath::Min(SFs->GetNbinsY(), SFs->GetYaxis()->FindBin(y)));
    return SFs->GetBinContent(xbin, ybin);
}


float WeightHandler::GetSFErrFromEGM(float x, float y){
    if (!SFs)
        throw std::runtime_error(Form("WeightHandler::GetSFErrFromEGM (%s): null histogram", name.c_str()));

    int xbin = TMath::Max(1, TMath::Min(SFs->GetNbinsX(), SFs->GetXaxis()->FindBin(x)));
    int ybin = TMath::Max(1, TMath::Min(SFs->GetNbinsY(), SFs->GetYaxis()->FindBin(y)));
    return SFs->GetBinError(xbin, ybin);
}


ROOT::RDF::RNode WeightHandler::GetRDF(ROOT::RDF::RNode df, std::string column, std::string column_err, std::vector<std::string> in_cloumns){
    if (in_cloumns.size() != 2 && in_cloumns.size() != 3)
        throw std::runtime_error(Form("WeightHandler::GetRDF (%s): input column size should be 2 or 3, (x, y, z=0)!", name.c_str()));
    
    if (SFs){
        auto nf = df.Define(column,     [this]( float x, 
                                                float y){
                                                    return WeightHandler::GetSFFromEGM(x, y);
                                                }, in_cloumns)
                    .Define(column_err, [this]( float x, 
                                                float y){
                                                    return WeightHandler::GetSFErrFromEGM(x, y);
                                                }, in_cloumns);
        return nf;
    }
    else if (values_2D.size() > 0){
        auto nf = df.Define(column,     [this]( float x, 
                                                float y){
                                                    return WeightHandler::GetSFFromHgg(x, y);
                                                }, in_cloumns)
                    .Define(column_err, [this]( float x, 
                                                float y){
                                                    return WeightHandler::GetSFErrFromHgg(x, y);
                                                }, in_cloumns);
        return nf;
    }
    else if (values_3D.size() > 0){
        auto nf = df.Define(column,     [this]( float x, 
                                                float y,
                                                float z){
                                                    return WeightHandler::GetSFFromHgg(x, y, z);
                                                }, in_cloumns)
                    .Define(column_err, [this]( float x, 
                                                float y,
                                                float z){
                                                    return WeightHandler::GetSFErrFromHgg(x, y, z);
                                                }, in_cloumns);
        return nf;
    }
    else
        throw std::runtime_error(Form("WeightHandler::GetRDF (%s): Please initial the WeightHandler by Init first!", name.c_str()));
