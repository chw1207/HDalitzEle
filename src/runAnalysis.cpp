...
ROOT::RDF::RNode FindGoodPho(ROOT::RDF::RNode df, bool isMC, std::string vary) {
    ...
    auto nf = ff.Define("isEBPho",              "abs(phoSCEta) < 1.4442 && phoIDMVA > -0.02")
                ...
                .Define("RphoIdx1",             [](Vec<int> good, Vec<float> pt){
                    if (ROOT::VecOps::Sum(good) > 0) 
                        return utils::getIdx(good, pt)[0]; 
                    else 
                        return -1;
                }, {"isGoodRPho", "phoCalibEt_NEW"})
                .Define("MphoIdx1",             [](Vec<int> good, Vec<float> pt){
                    if (ROOT::VecOps::Sum(good) > 0) 
                        return utils::getIdx(good, pt)[0]; 
                    else 
                        return -1;
                }, {"isGoodMPho", "phoCalibEt_NEW"});
...
ROOT::RDF::RNode FindGoodEle(ROOT::RDF::RNode df, const YAML::Node cfg, bool isMC, std::string vary){
    ...
    auto nf = ff.Define("isEBEle",              "abs(eleSCEta) < 1.4442")
                ...
                .Define("isReGoodEle",          "isMVAEle && (isEBEle || isEEEle)")
...
ROOT::RDF::RNode CheckGenerator(ROOT::RDF::RNode df, bool isMC){
    ...
    auto nf = df.Define("genIdx_reco1",         gen::MatchedGenEle,         {"ele1", "nMC", "mcEta", "mcPhi", "mcPID", "mcMomPID", "mcGMomPID", "mcStatusFlag"})
...
// template<typename T>
// ROOT::RVec<std::vector<T>> MakeMVAVals(const std::vector<ROOT::RVec<T>>& args){
    
    
    
//     const int n = args[0].size();
//     ROOT::RVec<std::vector<T>> outtrans;
//     for(int i = 0; i < n; i++) {   
//         outtrans.push_back({args[i]...});
//     }
//     return outtrans;
// };
...
void xAna(std::string inpath, std::string outpath, std::string era, YAML::Node cfg, int range, int iset){
    TStopwatch time_j;
    time_j.Start();
    ...
    printf("     - %s\n", inpath.c_str());
    auto df = ROOT::RDataFrame("ggNtuplizer/EventTree", inpath);
    ...
    if (isMC){
        ...
        // remove events with dielectron mass > 20 and photon is internally converted 
        ...
        if (!fs::exists(direc))
            gSystem->Exec(Form("mkdir -p %s", direc.c_str()));
    ...
    // =======================================================//
    bool UseSingleEleHLT = ba::contains(inpath, "SingleEle");
    ...
    std::string DoublePhoHLT = "((HLTPho >> 14) & 1) == 1";
    std::string HLT = "";
    if (UseSingleEleHLT)            HLT = SingleEleHLT;
    if (!UseSingleEleHLT && !isMC)  HLT = DoublePhoHLT; // only data 
    if (!HLT.empty()){
        printf(" [+] HLT trigger: \n");
        printf("     - %s \n", HLT.c_str());
        df_analysis = df_analysis.Filter(HLT, "trigger cut");
    }
    
    auto df1 = df_analysis.Filter("isPVGood == 1", "Good Vtx")
...
int main(int argc, char** argv){
    ...
    auto readpath = cfg["ntuple_path"].as<std::vector<std::string>>();
    ...
    for (size_t i = 0; i < readpath.size(); i++){
        printf("*************************  %ld  ************************\n", i+1);
        xAna(readpath[i], savepath[i], era, cfg, range, i);
    }
